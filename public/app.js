...
function baselineAuditPanel(report = {}, run = {}, compactMode = false) {
  ...
  return `<section class="card page-panel baseline-audit">
    <div class="card-head"><div><div class="card-title">Phase 2 scan audit</div><div class="card-kicker">Fail-closed security and market-quality evidence · ${auditValue(run.correlationId || "no scan yet")}</div></div><span class="badge badge-${String(report.qualityStatus || run.qualityStatus || "UNKNOWN").toLowerCase() === "full" ? "green" : "yellow"}">${esc(report.qualityStatus || run.qualityStatus || "NOT RUN")}</span></div>
...
function liveDashboard() {
  ...
  return head("Command center", "Research the evidence before the crowd.", "A baseline view of live DexScreener data, fail-closed Solana security checks, virtual execution, and scan auditability.", `<button class="btn btn-primary" onclick="scan()">↻ Scan now</button>`) +
    `<div class="grid metrics">${stat("Accepted candidates", sorted.length, "phase2-v1 gates passed", "◈", sorted.length ? "metric-positive" : "")}${stat("Whale net flow", money(latestWhale?.netFlow), "provider evidence only", "↗", latestWhale ? "metric-positive" : "")}${stat("Virtual equity", money(portfolio.equity), `${portfolio.roi >= 0 ? "+" : ""}${portfolio.roi.toFixed(2)}% total ROI`, "◫", portfolio.roi >= 0 ? "metric-positive" : "metric-negative")}${stat("Risk alerts", riskAlerts, "requiring review", "!", riskAlerts ? "metric-negative" : "")}</div>` +
    ...
    `<div class="token-detail token-profile-detail">
      ...
      <aside class="card evidence token-side-card">
        ...
        <ul class="security-reasons">${securityReasons.map(reason => `<li>${esc(reason)}</li>`).join("")}</ul>
         ...
         <div class="side-card-heading"><div><h3>Market-quality gate</h3><span>Phase 2 · $${marketMetrics.orderSizeUsd ?? 100} research entry proxy</span></div><span class="security-status ${marketQuality.status === "PASSED" ? "verified" : "rejected"}">${esc(marketQuality.status || "UNKNOWN")}</span></div>
