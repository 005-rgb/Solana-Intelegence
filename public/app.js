...
function liveDashboard() {
  ...
  return head("Command center", "Research the evidence before the crowd.", "A baseline view of live DexScreener data, fail-closed Solana security checks, virtual execution, and scan auditability.", `<button class="btn btn-primary" onclick="scan()">↻ Scan now</button>`) +
    ...
    `${whaleChart()}<div class="grid section-grid"><div class="card matrix"><div class="card-title">Baseline gate coverage</div><div class="card-kicker">Audit counters from the latest persisted scan</div><div class="health-row"><span>Provider records checked</span><strong>${snapshot.system?.securityFilter?.checked ?? 0}</strong></div><div class="health-row"><span>Accepted into board</span><strong class="health-ok">${snapshot.system?.securityFilter?.accepted ?? 0}</strong></div><div class="health-row"><span>Rejected / unresolved</span><strong>${(snapshot.system?.securityFilter?.rejected ?? 0) + (snapshot.system?.securityFilter?.unresolved ?? 0)}</strong></div><div class="data-note">No predictive score or threshold claim is shown in the baseline.</div></div><div class="card insight-card"><div class="insight-label">Score engine</div><div class="insight-number">UNKNOWN</div><div class="progress"><span style="width:0%"></span></div><div class="insight-note">Radar, opportunity, flow, risk, and confidence are not calculated in baseline-v1.</div></div><div class="card insight-card"><div class="insight-label">Scan health</div><div class="insight-number">${snapshot.lastScan ? "READY" : "IDLE"}</div><div class="progress"><span style="width:${snapshot.lastScan ? 100 : 35}%;background:var(--green)"></span></div><div class="insight-note">Server-side 15s scheduler · ${snapshot.lastScan ? `last completed ${new Date(snapshot.lastScan).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "run a scan to initialize"}</div></div></div>`;
}
