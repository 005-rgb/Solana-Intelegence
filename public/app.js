...
function baselineAuditPanel(report = {}, run = {}, compactMode = false) {
  ...
  return `<section class="card page-panel baseline-audit">
    <div class="card-head"><div><div class="card-title">Baseline-v1 scan audit</div><div class="card-kicker">Persisted denominators and fail-closed decision evidence · ${auditValue(run.correlationId || "no scan yet")}</div></div><span class="badge badge-${String(report.qualityStatus || run.qualityStatus || "UNKNOWN").toLowerCase() === "full" ? "green" : "yellow"}">${esc(report.qualityStatus || run.qualityStatus || "NOT RUN")}</span></div>
...
function radar() {
  ...
  const filterNotice = !items.length && report
    ...
    : "";
  return head("Discovery", "Baseline candidate board", "Review the current LIVE universe after baseline-v1 hard filters. Predictive scores remain UNKNOWN and are never estimated.", `<button class="btn btn-primary" onclick="scan()">↻ Scan now</button>`) +
    filterNotice +
