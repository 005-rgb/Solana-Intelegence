...
function baselineAuditPanel(report = {}, run = {}, compactMode = false) {
  ...
  return `<section class="card page-panel baseline-audit">
    <div class="card-head"><div><div class="card-title">Phase 0 baseline scan audit</div><div class="card-kicker">Fail-closed baseline evidence and immutable counters · ${auditValue(run.correlationId || "no scan yet")}</div></div><span class="badge badge-${String(report.qualityStatus || run.qualityStatus || "UNKNOWN").toLowerCase() === "full" ? "green" : "yellow"}">${esc(report.qualityStatus || run.qualityStatus || "NOT RUN")}</span></div>
...
function radar() {
  ...
  const filterNotice = !items.length && report
    ...
    : "";
   return head("Discovery", "Phase 0 baseline candidate board", "Review the current LIVE universe after the frozen fail-closed baseline filters. Predictive scores remain UNKNOWN and are never estimated.", `<button class="btn btn-primary" onclick="scan()">↻ Scan now</button>`) +
    filterNotice +
...
function smartMoney() {
  ...
  return head("Capital intelligence", "Flow evidence", "Review provider-reported flow separately. Wallet identity and a smart-money scorer are not available in baseline-v1.", `<button class="btn btn-primary" onclick="scan()">↻ Refresh signals</button>`) +
