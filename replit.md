...
- `GET /api/observability` provides the versioned `m0-observability-v1` baseline
  snapshot and safe provider/RPC summaries without triggering provider requests.
...
- Sell-route evidence is normalized from the persisted execution quote structure, including route status, price impact, slippage, fee fields, and invalidation codes. A route can be marked tradable while executable return remains `UNKNOWN` when either slippage or fee evidence is missing.
